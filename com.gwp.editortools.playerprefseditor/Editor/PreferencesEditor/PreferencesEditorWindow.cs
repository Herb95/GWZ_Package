...
using Gwp.EditorTools.PlayerPrefsEditor.Dialogs;
...
namespace Gwp.EditorTools.PlayerPrefsEditor
{
    public class PreferencesEditorWindow : EditorWindow
    {
        ...
        private readonly List<TextValidator> prefKeyValidatorList = new List<TextValidator>()
        {
            new TextValidator(TextValidator.ErrorType.Error,
                ...
                @"(^$)|(^[a-zA-Z0-9 ,.;:<>_|!§$%&/()=?*+~#-]+$)"),
            new TextValidator(TextValidator.ErrorType.Warning, @"The given key already exist. The existing entry would be overwritten!",
        ...
        private readonly char[] invalidFilenameChars = { '$', '%', '&', '\\', '/', ':', '<', '>', '|', '~' };
#endif
        [MenuItem("Tools/EditorTools/PlayerPrefs Editor", false, 1)]
 ...
 @"Library/Preferences/unity." + MakeValidFileName(PlayerSettings.companyName) + "." + MakeValidFileName(PlayerSettings.productName) + ".plist";
            ...
            monitoring = EditorPrefs.GetBool("Gwp.EditorTools.PlayerPrefsEditor.WatchingForChanges", true);
            if (monitoring)
                entryAccessor.StartMonitoring();
            sortOrder = (PreferencesEntrySortOrder)EditorPrefs.GetInt("Gwp.EditorTools.PlayerPrefsEditor.SortOrder", 0);
        ...
        private void InitReorderedList()
        {
            ...
            relSpliterPos = EditorPrefs.GetFloat("Gwp.EditorTools.PlayerPrefsEditor.RelativeSpliterPosition", 100 / position.width);
            ...
            userDefList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                {
                    case PreferenceEntry.PrefTypes.Float:
                        EditorGUI.DelayedFloatField(new Rect(rect.x + spliterPos + 62, rect.y, rect.width - spliterPos - 60, EditorGUIUtility.singleLineHeight),
                            value, GUIContent.none);
                        break;
                    case PreferenceEntry.PrefTypes.Int:
                        EditorGUI.DelayedIntField(new Rect(rect.x + spliterPos + 62, rect.y, rect.width - spliterPos - 60, EditorGUIUtility.singleLineHeight),
                            value, GUIContent.none);
                        break;
                    case PreferenceEntry.PrefTypes.String:
                        EditorGUI.DelayedTextField(new Rect(rect.x + spliterPos + 62, rect.y, rect.width - spliterPos - 60, EditorGUIUtility.singleLineHeight),
                            value, GUIContent.none);
                        break;
                }
        ...
        private void OnDrawElementBackgroundCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (Event.current.type == EventType.MouseUp)
            {
                ...
                EditorPrefs.SetFloat("Gwp.EditorTools.PlayerPrefsEditor.RelativeSpliterPosition", relSpliterPos);
            }
        }
        ...
        void OnGUI()
        {
            ...
            try
            {
                ...
                if (GUILayout.Button(sortOrderContent, EditorStyles.toolbarButton))
                {
                    ...
                    EditorPrefs.SetInt("Gwp.EditorTools.PlayerPrefsEditor.SortOrder", (int)sortOrder);
                    PrepareData(false);
                }
                ...
                if (GUILayout.Button(watcherContent, EditorStyles.toolbarButton))
                {
                    ...
                    EditorPrefs.SetBool("Gwp.EditorTools.PlayerPrefsEditor.WatchingForChanges", monitoring);
                    if (monitoring)
                        entryAccessor.StartMonitoring();
                    else
                        entryAccessor.StopMonitoring();
        ...
        private void CreatePrefEntries(string[] keySource, ref List<PreferenceEntry> listDest)
        {
            ...
            {
                listDest = listDest.Where((preferenceEntry) => preferenceEntry.ValueAsString().ToLower().Contains(searchTxt.ToLower()))
...
public class MySearchField : SearchField
{
    ...
    public new string OnToolbarGUI(string text, params GUILayoutOption[] options) =>
        this.OnToolbarGUI(GUILayoutUtility.GetRect(29f, 200f, 18f, 18f, EditorStyles.toolbarSearchField, options), text);

    public new string OnToolbarGUI(Rect rect, string text) =>
        this.OnGUI(rect, text, EditorStyles.toolbarSearchField, EditorStyles.toolbarButton, EditorStyles.toolbarButton);
}